...
**Human-authored pull requests targeting `main` must be raised through [`no-mistakes`](https://github.com/pixeloven/no-mistakes).**
...
1. Fork the repo, then clone the parent repo or set your local `origin` back to the parent (`git@github.com:pixeloven/operator.git`).
2. Create a branch and make your changes.
3. Initialize the gate with your fork as the push target: `no-mistakes init --fork-url git@github.com:<you>/operator.git` (contributing to `operator` requires **no-mistakes v1.46.0+** for structured attestation; without a fork, plain `no-mistakes init` still works for maintainers with push access).
...
See the [no-mistakes quick start](https://github.com/pixeloven/no-mistakes/blob/main/docs/src/content/docs/start-here/quick-start.md) for the full first-run walkthrough.
