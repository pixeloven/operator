...
An acknowledgement carries two separable facts: queue-row consumption is bound to the exact presentation claim identified by its actor, `--ack-through` boundary, and `--recovery-generation`, while only retiring the episode is bound to the current recovery generation.
A generation mismatch therefore permits consumption only when that older exact presentation claim still exists; it cannot consume rows claimed by a newer handler, and it remains a non-fatal result that names its own remedy - re-drain, then acknowledge the newer episode.
...
A successful raw presentation atomically records each exact row, actor, recovery generation, and presentation boundary in `state/.wake-presented-rows` before releasing that lock, so another drain in the same generation skips the row while a failed pre-commit presentation or a fresh interruption-recovery generation can replay it.
...
Its `--ack-through <SEQ>` identifies the exact main presentation whose rows may be deleted, while a branch acknowledgement deletes only the exact rows claimed by that branch presentation.
